    ...
    FullM="${mem_diff}M"
        if [ -f /root/memory_usage.sh ]; then
             echo "file found...开始执行 吃掉内存"
                cd /root && wget -qO memory_usage.sh https://raw.githubusercontent.com/xingyu2016/Oracle_OneKey_Active/main/memory_usage.sh && chmod +x memory_usage.sh && bash memory_usage.sh consume ${FullM}
        else
